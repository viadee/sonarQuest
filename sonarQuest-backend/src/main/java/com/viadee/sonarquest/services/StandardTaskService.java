...
public class StandardTaskService {
    ...
    public List<StandardTaskDTO> getByWorld(final World world) {
        List<StandardTask> tasks = standardTaskRepository.findByWorld(world);
        return findByWorld(tasks,world);
    }

    @Cacheable(value = "freeTaskScoringCache", key = "#world.id")
    public List<StandardTaskDTO> getFreeByWorld(final World world) {
        List<StandardTask> tasks = standardTaskRepository.findByWorldAndStatusAndQuestIsNull(world, SonarQuestStatus.OPEN);
        return findByWorld(tasks,world);
    }
    ...
    public List<StandardTaskDTO> updateGetByWorldCache(final World w) {
        LOGGER.info("Cache 'taskScoringCache' for world '{}' has been updated", w.getName());
        return getByWorld(w);
    }

    @CachePut(value = "freeTaskScoringCache", key = "#w.id")
    public List<StandardTaskDTO> updateGetFreeByWorldCache(final World w) {
        LOGGER.info("Cache 'freeTaskScoringCache' for world '{}' has been updated", w.getName());
        return getFreeByWorld(w);
    }
