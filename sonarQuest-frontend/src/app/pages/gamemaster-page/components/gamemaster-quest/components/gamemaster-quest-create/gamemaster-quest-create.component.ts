...
export class GamemasterQuestCreateComponent implements OnInit {
  ...
  createQuest(){
    if(this.title && this.gold && this.xp && this.story && this.world && (this.tasks.length != 0)){
      let quest = {
        title: this.title,
        gold: this.gold,
        xp: this.xp,
      ...
      this.questService.createQuest(quest).then((quest)=>{
        if(quest.id){
          ...
          let addQuestToWorld = this.questService.addToWorld(quest,this.world);
  ...
  addFreeTask(){
    this.dialog.open(GamemasterAddFreeTaskComponent,{data:[this.world,this.tasks]})
  ...
  suggestTasks(){
    this.dialog.open(GamemasterSuggestTasksComponent,{data:[this.world,this.tasks]}).afterClosed().subscribe(result=>{
      if(result){
