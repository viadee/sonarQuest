...
export class GamemasterSpecialTaskComponent implements OnInit {
  ...
  loadTasks() {
    this.specialTaskService.getSpecialTasksForWorld(this.currentWorld).subscribe(tasks => {
      this.data = tasks;
      this.filter();
    });
  }
  ...
  newSpecialTask() {
    this.dialog.open(GamemasterSpecialTaskCreateComponent, {
  ...
  editSpecialTask(specialTask) {
    this.dialog.open(GamemasterSpecialTaskEditComponent, {
      width: '500px',
      data: specialTask
